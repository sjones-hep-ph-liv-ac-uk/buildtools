...
while(<INPUTFILE>) {
  ...
  if ($lineIn =~ /^\s*\#?\s*host\s+(\d+\-\d+)\s+.*$/) {
    ...
    if (defined($rackNodeMap{$rackNodeTag})) {
      ...
      @hostSectionLines = editHostSection(@hostSectionLines);
    }
...
#---------------------------------------------
# Make edits to a host section 
#---------------------------------------------
sub editHostSection (@) {
  
  my @hostSection = @_;
  
  # Maybe remove any existing NEXTSERVER or BOOTFILE entries
  if ($parameter{'REMOVE'} > 0 ) {
    @hostSection = removeBootFileEntries(@hostSection);
    @hostSection = removeNextServerEntries(@hostSection);
  }
  ...
  if (defined($parameter{'NEXTSERVER'})) {
    ...
    my $lastLine = pop(@hostSection);

    # Put in the next-server clause, then restore the bracket        
    push(@hostSection,"   next-server $parameter{'NEXTSERVER'}");
    push(@hostSection,$lastLine);
  }
  ...
  if (defined($parameter{'BOOTFILE'})) {
    ...
    push(@hostSection,"   filename   \"$parameter{'BOOTFILE'}\"");
    push(@hostSection,$lastLine);
  }

  # Finally, is the whole string to be enabled/disabled?
  if ($parameter{'MODE'} eq 'disable') {
    @hostSection = disableHostSection(@hostSection);
  }

  if ($parameter{'MODE'} eq 'enable') {
    @hostSection = enableHostSection(@hostSection);
  }
  return @hostSection;
}
...
sub mySort() {
  ...
  # Break up key A
  $a =~ /(\d+)\-(\d+)/;
  my $aPart1 = $1; my $aPart2 = $2;

  # Break up key B
  $b =~ /(\d+)\-(\d+)/;
  my $bPart1 = $1; my $bPart2 = $2;

  # Key comparisons.

  # Compare the first parts
  return  1 if ($aPart1 > $bPart1);
  return -1 if ($aPart1 < $bPart1);

  # Hm... the first parts must be equal. So
  # it depends on the 2nd parts
  return  1 if ($aPart2 > $bPart2);
  return -1 if ($aPart2 < $bPart2);

  # Hm... the 2nd parts are equal too. Whole thing is equal.
  return 0;
}
...
and to insert or remove next-server and filename lines for each host section.
  ...
  -n  --nodespecs    1/2      Specifies racks and nodes. Can also use lists, ranges and asterixes
  -m  --mode         enable   Should be enable or disable (takes the host section out)
  -ns --next-server  ipaddr   Inserts a next-server line in each host section
...
TEXT
  ...
  if (@{$parameter{'NODESPECS'}} <= 0) {
    die("You didn't specify what racks and nodes to process\n");
  }
  
  # Find which racks and nodes were specified
  foreach my $n (@{$parameter{'NODESPECS'}}) {
    ...
    # This map will be consulted whenever a node is found in the dhcpd.conf file.
    # If it exists in the map, edits will be made to it.
    foreach my $r (@racks) {
      foreach my $n (@nodes) {
        my $rackNodeKey = $r . "-" . $n;
  ...
  # Is the dhcpd.conf file there? Can we open it to read it?
